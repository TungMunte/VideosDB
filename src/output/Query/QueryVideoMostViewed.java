...
public class QueryVideoMostViewed extends Query {
    @Override
    public Result query(ActionInputData actionInputData, Input input) {
        ...
        for (int i = 0; i < storeNameShow.size(); i++) {
            ...
            for (UserInputData userInputData : input.getUsers()) {
                if (userInputData.getHistory().containsKey(storeNameShow.get(i)) == true) {
                    sum += userInputData.getHistory().get(storeNameShow.get(i));
                }
            }
            if (sum != 0) {
                unsortedNameShow.put(storeNameShow.get(i), sum);
            }
        }
